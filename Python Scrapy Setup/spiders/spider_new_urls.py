...
# Tool for using Javascript in the Scraper
...
class NewURLsSpider(scrapy.Spider):
    ...
    def __init__(self, source = 0, test = 0, *args, **kwargs):
        ...
        # Start the Selenium Javascript Driver
    ...
    def start_requests(self):
        ...
        self.logger.critical("CONCURRENT_REQUESTS_PER_IP: %s", self.settings['CONCURRENT_REQUESTS_PER_IP'])
        # Now loop through the urls / links listed in start_urls in
        # the xpath dictionary as set in the __init__ and start
        # scraping.
        for link in self.xpath_dict[self.source]['start_urls']:
